...
def GeneratePlayerInfo(justnew=False):
    if justnew:
        sql = """SELECT DISTINCT(adjs.player2_id) 
        FROM nhlstats.adjusted_shots adjs
        LEFT JOIN nhlstats.players p ON adjs.player2_id = p.id
        WHERE p.id IS NULL """
        allPlayerIds = pd.read_sql_query(sql, con=engine)
        allPlayerIds = allPlayerIds.rename(index=str, columns={"player2_id": "player_id"})

        sql = """SELECT DISTINCT(adjs.player1_id) 
                FROM nhlstats.adjusted_shots adjs
                LEFT JOIN nhlstats.players p ON adjs.player2_id = p.id
                WHERE p.id IS NULL """
        ...
        sql = """SELECT DISTINCT(adjs.player3_id) 
                FROM nhlstats.adjusted_shots adjs
                LEFT JOIN nhlstats.players p ON adjs.player2_id = p.id
                WHERE p.id IS NULL """
        ...
        sql = """SELECT DISTINCT(adjs.player4_id) 
                FROM nhlstats.adjusted_shots adjs
                LEFT JOIN nhlstats.players p ON adjs.player2_id = p.id
                WHERE p.id IS NULL """
    ...
    for index, row in allPlayerIds.iterrows():
        if (row['player_id'] == 0) | (row['player_id'] is None) | (math.isnan(row['player_id'])):
            print('skipping')
            print(row['player_id'], index)
            continue
...
if __name__ == '__main__':
    ...
    # GeneratePlayerInfo(justnew=False)
