...
def GenerateAndPushTeamAgainstSummaries(gameSeason):
    ...
    print(time.time() - s)
    print('fetching game details data...')
    s = time.time()
    if gameSeason == "all":
        sql = """SELECT gamepk, away_id, home_id FROM nhlstats.game_details"""
        gameDetails = pd.read_sql_query(sql, con=engine)
    else:
        sql = """SELECT gamepk, away_id, home_id FROM nhlstats.game_details
            WHERE game_season = %s""" % gameSeason
        gameDetails = pd.read_sql_query(sql, con=engine)


    print(time.time() - s)
    print('Join together details and shots...')
    s = time.time()
    allShots = allShots.merge(gameDetails, left_on=['game_id'], right_on=['gamepk'], how='inner', suffixes=('', '_details'))


    def get_against_team_id(row):
        if (row['home_id'] == row['team_id']):
            return row['away_id']
        return row['home_id']
    ...
    # allShots['shooting_team_id'] = allShots.apply(lambda row: get_shooting_team_id(row), axis=1)
    allShots['shooting_against_team_id'] = allShots.apply(lambda row: get_against_team_id(row), axis=1)
    ...
    shotsGrouped = tempShots.groupby(['shooting_against_team_id', 'game_season', 'month', 'game_type'])

    print(time.time() - s)
    print('Calculating grouped metrics per team...')
    s = time.time()
    ...
    metrics = metrics.merge(counts, on=['shooting_against_team_id', 'game_season', 'month', 'game_type'], how='inner', suffixes=('', '_count'))
    ...
    print(time.time() - s)
    print('Converting from shooter stats to team against metrics...')
    s = time.time()
    columns = {
    ...
    tempWithYearly['shooting_against_team_id'] = pd.to_numeric(tempWithYearly['shooting_against_team_id'], downcast='integer')

    shooting_against_team_ids = tempWithYearly['shooting_against_team_id']
    years = tempWithYearly['year_code']
    gameTypes = tempWithYearly['game_type']
    months = tempWithYearly['month']

    grouped = tempWithYearly.drop(columns=['shooting_against_team_id']).groupby(['year_code', 'game_type', 'month'])
    ...
    shooting_against_team_ids = shooting_against_team_ids.to_frame().set_index(ranks.index)
    ...
    ranks['shooting_against_team_id'] = shooting_against_team_ids
    ranks['year_code'] = years
    ranks['game_type'] = gameTypes
    ranks['month'] = months

    pctile_ranks['shooting_against_team_id'] = shooting_against_team_ids
    ...
    s = time.time()
    allRanks = pctile_ranks.merge(ranks, on=['shooting_against_team_id', 'year_code', 'month', 'game_type'], how='inner',
    ...
    s = time.time()
    if gameSeason == "all":
        sql = """DELETE from nhlstats.yearly_team_against_summaries"""
    else:
        sql = """DELETE FROM nhlstats.yearly_team_against_summaries
    ...
    cols = {'shooting_against_team_id': 'id'}
    ...
    # null values become ''
    cursor.copy_from(output, 'nhlstats.yearly_team_against_summaries', null="", columns=(columns))
    connection.commit()
    cursor.close()
    ...
    s = time.time()
    if gameSeason == "all":
        sql = """DELETE from nhlstats.yearly_team_against_ranks"""
    else:
        sql = """DELETE FROM nhlstats.yearly_team_against_ranks
                                   WHERE year_code = %s""" % gameSeason
    connection = engine.connect()
    ...
    cursor = connection.cursor()
    # null values become ''
    cursor.copy_from(output, 'nhlstats.yearly_team_against_ranks', null="", columns=(columns))
    connection.commit()
    cursor.close()
    print(time.time() - s)
...
if __name__ == '__main__':
    ...
    GenerateAndPushTeamAgainstSummaries(sys.argv[1])
    # GenerateAndPushTeamAgainstSummaries("all")
    # GenerateAndPushTeamAgainstSummaries("20182019")