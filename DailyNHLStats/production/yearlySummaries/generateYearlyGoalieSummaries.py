...
def GenerateAndPushGoalieSummaries(gameSeason):
    ...
    # null values become ''
    cursor.copy_from(output, 'nhlstats.yearly_goalie_summaries', null="", columns=(columns))
    connection.commit()
    cursor.close()
    ...
    cursor = connection.cursor()
    # null values become ''
    cursor.copy_from(output, 'nhlstats.yearly_goalie_ranks', null="", columns=(columns))
    connection.commit()
    cursor.close()
    print(time.time() - s)
