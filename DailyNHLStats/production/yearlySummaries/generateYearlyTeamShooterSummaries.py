...
def GenerateAndPushTeamShooterSummaries(gameSeason):
    ...
    s = time.time()
    allShots = allShots.merge(gameDetails, left_on=['game_id'], right_on=['gamepk'], how='inner',
                              suffixes=('', '_details'))


    def get_shooting_team_id(row):
        if(row['away_id'] == row['team_id']):
            return row['away_id']
        return row['home_id']
    ...
    shotsGrouped = tempShots.groupby(['shooting_team_id', 'game_season', 'month', 'game_type'])
    ...
    metrics = metrics.merge(counts, on=['shooting_team_id', 'game_season', 'month', 'game_type'], how='inner', suffixes=('', '_count'))
    ...
    grouped = tempWithYearly.drop(columns=['shooting_team_id']).groupby(['year_code', 'game_type', 'month'])
    ...
    ranks['shooting_team_id'] = playerIds
    ranks['year_code'] = years
    ranks['game_type'] = gameTypes
    ranks['month'] = months

    pctile_ranks['shooting_team_id'] = playerIds
    ...
    allRanks = pctile_ranks.merge(ranks, on=['shooting_team_id', 'year_code', 'month', 'game_type'], how='inner',
    ...
    if gameSeason == "all":
        sql = """DELETE from nhlstats.yearly_team_shooter_summaries"""
    else:
        sql = """DELETE FROM nhlstats.yearly_team_shooter_summaries
    ...
    # null values become ''
    cursor.copy_from(output, 'nhlstats.yearly_team_shooter_summaries', null="", columns=(columns))
    connection.commit()
    cursor.close()
    print(time.time() - s)
    ...
    if gameSeason == "all":
        sql = """DELETE from nhlstats.yearly_team_shooter_ranks"""
    else:
        sql = """DELETE FROM nhlstats.yearly_team_shooter_ranks
                                   WHERE year_code = %s""" % gameSeason
    connection = engine.connect()
    ...
    cursor = connection.cursor()
    # null values become ''
    cursor.copy_from(output, 'nhlstats.yearly_team_shooter_ranks', null="", columns=(columns))
    connection.commit()
    cursor.close()
    print(time.time() - s)
...
if __name__ == '__main__':
    ...
    GenerateAndPushTeamShooterSummaries(sys.argv[1])
    # GenerateAndPushTeamShooterSummaries("all")
    # GenerateAndPushTeamShooterSummaries("20182019")
